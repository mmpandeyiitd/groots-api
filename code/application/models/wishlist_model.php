...
class wishlist_model extends CI_Model {
     ...
     * @return boolean
     */
    public function saveWishlistData($params) {
        try {
            $this->dbmycart->set('modified_at', 'NOW()', FALSE);
            if ($this->dbmycart->insert('wishlist', $params)) {
     ...
     * @return boolean
     */
    public function updateWishlistData($params){
       try{
            $this->dbmycart->set('modified_at', 'NOW()', FALSE);
            $this->dbmycart->where('id',  $params['id']);
            $this->dbmycart->update('wishlist',  $params);
    ...
    public function checkUserExists($usr_id){
       try{
            $user_id_query = $this->dbmycart->query('SELECT id,cart_data FROM wishlist where user_id="'.$usr_id.'"');
    ...
    public function checkSessionExists($session_id){
       try{
            $session_id_query = $this->dbmycart->query('SELECT id,cart_data FROM wishlist where session_id="'.$session_id.'"');
    ...
    public function getDataByUserId($user_id){
       try{
            $user_id_query = $this->dbmycart->query('SELECT id,cart_data FROM wishlist where user_id="'.$user_id.'"');
    ...
    public function getDataBySessionId($session_id){
     ...
     public function deleteWishlistByCartId($cart_id) {
        try {
            $this->dbmycart->query('DELETE FROM wishlist where id="' . $cart_id . '"');
            if ($this->dbmycart->_error_message()) {
                $dberrorObjs->error_code = $this->dbmycart->_error_number();
                $dberrorObjs->error_message = $this->dbmycart->_error_message();
                $dberrorObjs->error_query = $this->dbmycart->last_query();
