...
class user_model extends CI_Model {
    ...
    public function getUserDetails($params) {
       try{
            $query = 'SELECT r.id, r.name as retailerName,r.name as orgName,r.retailer_grade_type as retailer_grade_type,r.business_type as business_type,r.email as email, r.mobile as contact,r.address as address,r.alternate_email as alternateEmail,r.city as city,r.state as state,r.website as website,r.pincode as pincode,r.tan_no as tanNo,r.pan_no as panNo,r.payment_mode as paymentMode,r.collection_frequency as paymentFreq,r.min_order_price as minOrderPrice, r.due_date as outstandingDate, r.total_payable_amount as outstandingAmount, c.name as collectionRepName, ge.name as salesRepName  FROM retailer as r left join collection_agent as c on r.collection_agent_id = c.id left join groots_employee as ge on r.sales_rep_id = ge.id where r.id="'.$params['user_id'].'"';
            //echo $query; die;
